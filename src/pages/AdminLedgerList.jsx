...
const HistoryTable = ({ history }) => {
  ...
  return (
    <table className="history-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr>
          <th style={{ textAlign: 'left', padding: '4px 8px', borderBottom: '1px solid rgba(255,255,255,0.2)' }}>사용자</th>
          <th style={{ textAlign: 'left', padding: '4px 8px', borderBottom: '1px solid rgba(255,255,255,0.2)' }}>타입</th>
          <th style={{ textAlign: 'left', padding: '4px 8px', borderBottom: '1px solid rgba(255,255,255,0.2)' }}>프로젝트</th>
          <th style={{ textAlign: 'left', padding: '4px 8px', borderBottom: '1px solid rgba(255,255,255,0.2)' }}>날짜</th>
        </tr>
      </thead>
      <tbody>
        {history.map((item, index) => (
          <tr key={`${item.username ?? 'unknown'}-${index}`}>
            <td style={{ padding: '4px 8px' }}>{item.username ?? '-'}</td>
            <td style={{ padding: '4px 8px' }}>{item.type ?? '-'}</td>
            <td style={{ padding: '4px 8px' }}>{item.projectName ?? '-'}</td>
